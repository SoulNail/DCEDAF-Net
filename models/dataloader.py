...
def is_image(src):
    return True if os.path.splitext(src)[1].lower() in ['.jpg', '.png', '.tif', '.ppm', '.bmp'] else False
...
class Image2ImageLoader_zero_pad(Dataset):
    ...
    def transform(self, image, target):
        ...
        if self.args.input_space == "L":
            # 将RGB图像转换为灰度图
            image_tensor_grey = tf.to_tensor(tf.to_grayscale(image))
            image_tensor = image_tensor_grey
    ...
    def __getitem__(self, index):
        img_x_tr, img_y_tr = self.transform(self.img_x[index], self.img_y[index])       # self.img_x 和 self.img_y 分别是存储输入图像和目标图像的列表或数组。
                                                                                        # 通过 index 取出第 index 个输入图像 self.img_x[index] 和对应的目标图像 self.img_y[index]。
