...
from models.backbones import RRWNet as RRWNet_parts
from models.backbones.MAGFNet import magfnet as MAGFNet_parts
from models.backbones import IPUNet as IPUNet_parts
from models.backbones.ParaTransCNN import ParaTransCNN as ParaTransCNN_parts
from models.backbones.vmunet import vmunet as vmunet_parts
from models.backbones import MyModel
...
class RRWNet(nn.Module):
    def __init__(self, in_channels, n_classes, **kwargs):
        super().__init__()
        self.RRWNet = RRWNet_parts.RRWNet(input_ch=in_channels, output_ch=3, base_ch=64, iterations=5)

    def forward(self, x):
        return self.RRWNet(x)

class MAGFNet(nn.Module):
    def __init__(self, in_channels, n_classes, **kwargs):
        super().__init__()
        self.MAGFNet = MAGFNet_parts.MagfNet(in_channels=in_channels, num_classes=n_classes, features_dim=64)

    def forward(self, x):
        return self.MAGFNet(x)

class IPUNet(nn.Module):
    def __init__(self, in_channels, n_classes, **kwargs):
        super().__init__()
        self.IPUNet = IPUNet_parts.IPUNet(img_ch=in_channels, output_ch=n_classes)

    def forward(self, x):
        return self.IPUNet(x)


class ParaTransCNN(nn.Module):
    def __init__(self, in_channels, n_classes, **kwargs):
        super().__init__()
        self.ParaTransCNN = ParaTransCNN_parts.ParaTransCNN(n_channels=in_channels, num_classes=n_classes, heads=8, dim=320, depth=(3, 3, 3), patch_size=2)

    def forward(self, x):
        return self.ParaTransCNN(x)


class vmunet(nn.Module):
    def __init__(self, in_channels, n_classes, **kwargs):
        super().__init__()
        self.vmunet = vmunet_parts.VMUNet(input_channels=in_channels, num_classes=n_classes, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2], drop_path_rate=0.2)

    def forward(self, x):
        return self.vmunet(x)


class MyModel_modify(nn.Module):
    def __init__(self,
                 in_channels=3,
                 n_classes=1,
                 depths=[3, 3, 9, 3],
                 ...
                 **kwargs):
        super().__init__()
        self.MyModel = MyModel.DCEDAFNet(in_channels, n_classes, base_c,
