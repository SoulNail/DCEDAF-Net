...
class Trainer_seg:
    def __init__(self, args, now_time=None):
        ...
        frozen_params = (total_params - trainable_params) / 1e6
    ...
    def _train(self, epoch):
        ...
        for batch_idx, (x_in, target) in enumerate(self.loader_train.Loader):
            ...
            # 多尺度监督output 如果是 tuple 或 list（即 out_1, out_2, out_3），则：
    ...
    def init_model(model_name, device, args):
        # 创建模型实例
        model = getattr(model_implements, model_name)(**vars(args))

        # 获取模型的输入通道数（从模型的第一层）
        # 这是一个简单的尝试，可能需要根据您的模型结构进行调整
        input_channels = 3  # 默认为3通道
        try:
            # 尝试检查模型的第一个卷积层以获取正确的输入通道数
            for module in model.modules():
                if isinstance(module, torch.nn.Conv2d):
                    input_channels = module.in_channels
                    break
        except:
            print("无法检测输入通道数，使用默认值3")
        ...
        # 尝试计算FLOPS和参数数量
        try:
            # 创建与模型输入相匹配的虚拟输入，使用检测到的通道数
            dummy_input = torch.randn(1, 3, 288, 288).to(device)

            # 禁用thop的hooks以避免设备不匹配问题
            with torch.no_grad():
                # 使用FLOPs计数器
                from thop import profile
